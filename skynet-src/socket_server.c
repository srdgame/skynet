...
int 
socket_server_udp_sendto(struct socket_server *ss, const char *addr, int port, struct socket_sendbuffer *buf) {
	uint8_t address[UDP_ADDRESS_SIZE];
	int status;
	struct addrinfo ai_hints;
	struct addrinfo *ai_list = NULL;
	...
	return socket_server_udp_send(ss, (const struct socket_udp_address *)address, buf);
}
